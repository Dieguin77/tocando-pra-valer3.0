    ...
    <div className="min-h-screen bg-slate-900 text-white font-sans selection:bg-indigo-500 selection:text-white">
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
        ...
        <div className="relative max-w-2xl mb-16 group">
          ...
          <div className="relative flex items-center bg-slate-800 rounded-xl border border-slate-700 shadow-2xl">
            ...
            <input 
              ...
              className="w-full bg-transparent p-4 text-white placeholder-slate-500 focus:outline-none focus:ring-0 text-lg"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredSongs.map((song) => (
            <Link 
              to={`/musica/${song.id}`}
              key={song.id} 
              className="group relative bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 hover:bg-slate-800 transition-all duration-300 hover:shadow-[0_0_30px_rgba(79,70,229,0.15)] hover:-translate-y-1 cursor-pointer"
            >
              <div className="flex justify-between items-start mb-4">
                ...
                <PlayCircle className="w-8 h-8 text-slate-600 group-hover:text-white transition-colors opacity-0 group-hover:opacity-100" />
              </div>
              ...
              <div className="flex items-center gap-2">
                <span className="px-3 py-1 bg-slate-900 rounded-full text-xs font-semibold text-slate-300 border border-slate-700">
                  Tom: {song.tom}
                </span>
              </div>
            </Link>
        ...
        {filteredSongs.length === 0 && (
          <div className="text-center py-20">
            <p className="text-slate-500 text-lg">Nenhuma música encontrada para "{searchTerm}"</p>
          </div>
        )}
      </div>
    </div>
